...
function idcheck() {
	//id 텍스트 상자가 비어 있으면 "아이디 입력"
	fid = encodeURIComponent(document.fr.id.value);//한글 변환encodeURIComponent("한글")
	...
	if(fid == ""){
		...
		//새창 열기 window.open("파일이름", "창이름","크기")	
		window.open("idcheck.jsp?fid="+fid, "","width=400, height=200");
	}
}

function check() {
	if(document.fr.id.value.length < 3  || document.fr.id.value.length > 7){
		alert('아이디는 3~7자리입니다.');
		document.fr.id.focus();
		return false;
	}
