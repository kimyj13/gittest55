...
int count = bdao.getBookCount(search);//검색어를 포함한 검색글 가져오기
...
if(count != 0){
BookList = bdao.getBookList(startRow, pageSize, search);
}
...
if(count != 0){
	...
	if(startPage > pageBlock){
		%><a href="booknoticeSearch.jsp?pageNum=<%=startPage - pageBlock%>&search=<%=search%>">Prev</a><%
	}
	for(int i = startPage; i <= endPage; i++ ){
		%><a href="booknoticeSearch.jsp?pageNum=<%=i%>&search=<%=search%>")><%=i %></a><%
	}
	if(endPage < pageCount){
		%><a href="booknoticeSearch.jsp?pageNum=<%=endPage+1%>&search=<%=search%>"><!-- endPage+1 -->Next</a><%
	}
}
